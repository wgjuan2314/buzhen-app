...
function renderTextWithBrackets(text, isUser = false) {
  ...
  // 根據消息類型選擇顏色（調得更淡、更優雅）
  const bracketColor = isUser ? 'text-[#1A1A1A]/40' : 'text-[#333]/50'
...
const ChatPage = forwardRef(function ChatPage({ onAutoGreeting, isMuted, toggleMute }, ref) {
  ...
  return (
    <>
      {/* 層級一：頂部導航欄 (Navbar) */}
      <div className="fixed left-0 top-0 z-[120] w-full pt-[env(safe-area-inset-top)]">
        <div className="mx-auto flex max-w-md items-center justify-end px-4 py-3 settings-nav-container">
          {/* 右側：設置圖標 */}
          <button
            className="flex h-8 w-8 items-center justify-center text-[#333] transition-all hover:opacity-80 active:scale-95 settings-icon-button"
      ...
      {/* 層級二：頂部懸浮內容區 (Top Floating Content) - z-index: 120 確保在 chat-container 之上 */}
      <div className="fixed left-0 top-0 z-[120] w-full pt-[env(safe-area-inset-top)] mt-[36px]">
        <div className="mx-auto flex max-w-md items-center gap-3 px-4">
          {/* 左側膠囊：角色信息 */}
          <div className="chat-header flex shrink-0 min-w-[140px] h-12 items-center gap-2 rounded-full px-2 py-2">
            {/* 頭像容器 */}
            ...
            <div className="flex flex-col">
              <div className="text-[12px] font-medium text-[#333]">江予白</div>
              <div className="text-[9px] text-[#666]">你的梦境守护者</div>
            </div>
            {/* 加號圖標（容器右端，垂直居中） */}
            <div className="ml-auto flex h-4 w-4 items-center justify-center">
              <CirclePlus className="h-4 w-4 text-[#333]" strokeWidth={2} />
            </div>
          </div>

          {/* 右側膠囊：親密度 */}
          <div className="chat-header flex shrink-0 min-w-[120px] h-12 items-center rounded-full px-2 py-2">
            {/* 左側：愛心圖標（帶等級數字） */}
            <div className="relative shrink-0">
              <svg
                className="h-6 w-6"
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {/* 第二行：進度條（高保真還原） */}
                    <div className="mt-0.5 h-[4px] w-[80%] overflow-hidden rounded-full bg-[#FF9EB5]/20">
                      <motion.div
                        className="h-full rounded-full"
                        style={{
                          background: 'linear-gradient(247.41deg, #FF9EB5 14.69%, #FF9EB5 82.67%)',
                        }}
                        initial={{ width: '0%' }}
      ...
      >
        ...
        <div className="mx-auto flex h-full w-full max-w-md flex-col relative">
          ...
          <div
            ref={listRef}
            className="message-list flex-1 overflow-y-auto px-4 py-4 relative"
            style={{
                ...
                {messages.map((m, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`message-slide-in ${m.role === 'user' ? 'mb-8 flex items-end gap-2' : 'mb-8 flex flex-col items-start'} ${isFirstAssistantMessage ? 'mt-[20px]' : ''}`}
                    >
                    ...
                    <div className={m.role === 'assistant' ? 'relative' : 'ml-auto'}>
                      ...
                      {m.role === 'assistant' && m.content && m.content.trim() && (() => {
                        ...
                        return (
                          <button
                            ...
                            className="voice-pill absolute left-0 -top-[12px] z-10 flex items-center gap-1.5 rounded-full px-2 py-1 transition-all hover:opacity-80 active:scale-95"
                            type="button"
                            aria-label="播放語音"
                          >
                            {playingAudioId === m.id ? (
                              ...
                              <>
                                <Play className="play-icon h-3 w-3 shrink-0" strokeWidth={2} stroke="#7B4744" fill="#7B4744" />
                                <span className="duration-text text-[10px] whitespace-nowrap">{durationText}</span>
                              </>
                            ) : (
                              <>
                                <Play className="play-icon h-3 w-3 shrink-0" strokeWidth={2} stroke="#7B4744" fill="#7B4744" />
                      ...
                      <div
                        className={
                          m.role === 'user'
                            ? `message-user max-w-[318px] p-4 text-left text-sm ${
                                isSingleLine(m.content) 
                                  ? 'rounded-full' 
                                  : 'rounded-[20px]'
                              }`
                            : `message-ai relative max-w-[318px] p-4 text-left text-sm ${
                                isSingleLine(m.content) 
                                  ? 'rounded-full' 
                                  : 'rounded-[20px]'
                              }`
              ...
              {isVoiceMode ? (
                ...
                <div className="input-wrapper flex-1 flex items-center">
                  <textarea
                    ...
                    placeholder="发消息给江予白..."
                    rows={1}
                    className="w-full h-10 max-h-32 resize-none rounded-full px-4 py-0 text-sm leading-[40px] text-[#333] placeholder:text-[#999] placeholder:leading-[40px] outline-none border-none transition-all"
                    style={{
              ...
              {!isVoiceMode && (
                ...
                >
                  <div className="send-icon-wrapper">
                    ...
                    >
                      <defs>
                        {/* 渐变定义：设计稿同款渐变 linear-gradient(220.73deg, #FF5456 13.67%, #FF54A7 83.77%) */}
                        <linearGradient id="send-gradient" x1="0%" y1="0%" x2="100%" y2="100%" gradientUnits="userSpaceOnUse">
                          <stop offset="13.67%" stopColor="#FF5456" />
                          <stop offset="83.77%" stopColor="#FF54A7" />
                        </linearGradient>
                        {/* SVG Filter：内阴影高光效果 */}
                        <filter id="send-inner-shadow" x="-50%" y="-50%" width="200%" height="200%">
                          <feOffset dx="-1" dy="3" in="SourceAlpha" result="offset" />
                          <feGaussianBlur stdDeviation="2" in="offset" result="blur" />
                          <feComposite in="SourceAlpha" in2="blur" operator="out" result="inner-shadow-mask" />
                          <feFlood floodColor="#FFFFFF" floodOpacity="0.5" result="flood" />
